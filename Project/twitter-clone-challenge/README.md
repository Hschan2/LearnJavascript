...
## 어려웠던 점
#### 파이어베이스 데이터베이스 규칙 설정
파이어베이스 데이터베이스를 사용하기 위한 규칙을 설정하는 과정에서 많은 어려움을 겪었습니다. 예를 들어, 이름과 이메일 중복 확인을 위한 데이터 저장에 필요한 데이터베이스 규칙에서 읽기와 쓰기에 대한 규칙을 어떻게 설정해야 할지, 그리고 팔로우와 팔로워 데이터를 저장하기 위해 각각 규칙에 대해 유저 ID를 사용하는 데 있어서 어려움을 겪었습니다. 팔로우와 달리 팔로우한 유저에게는 팔로워가 되는 경우에 쓰기 규칙을 어떻게 설정해야 문제없이 데이터를 저장할 수 있는 허가를 얻을 수 있는지 시간이 많이 필요했습니다.
